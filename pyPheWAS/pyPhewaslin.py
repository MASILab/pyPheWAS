...
def plot_data_points(x, y, thresh, dstring, save):
	"""
	...
	plt.ylabel('-log10(p)')
	plt.suptitle(dstring, x=.02, y=.98, horizontalalignment='left', verticalalignment='center')
	if save:
...
# remove defaults at some point, handled in the object
def phewas(path, filename, groupfile,covariates,save='',output=''):
	...
	dstring = ' / '.join([filename, groupfile, 'linear', covariates, str(thresh)])
	plot_data_points(results[0],results[1],-math.log10(thresh), dstring, save)
